...
export default function Products() {
  ...
  useEffect(() => {
    const fetchMostPopular = async () => {
      try {
        // Gọi BE: GET /api/products/most-popular?limit=10
        const res = await productApi.getMostPopular(10);
        setProducts(res.data);
        // Nếu bạn đã sửa productApi trả thẳng data:
        // const data = await productApi.getMostPopular(10);
        // setProducts(data);
      } catch (err) {
        console.error(err);
      ...
      <section className="flat-spacing-3 overflow-hidden">
        <div className="container">
          <div className="flat-title wow fadeInUp">
            <h4 className="title">Phổ biến nhất</h4>
          </div>

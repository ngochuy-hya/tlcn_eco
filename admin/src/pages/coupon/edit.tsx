...
export const CouponEdit = () => {
  const { formProps, saveButtonProps } = useForm<Coupon>({
    resource: "coupons",
    meta: {
      // 👇 đảm bảo refine chuyển string ISO -> Date khi hiển thị form
      transform: (values: any) => ({
        ...values,
        startAt: values.startAt ? dayjs(values.startAt) : null,
        endAt: values.endAt ? dayjs(values.endAt) : null,
      }),
    },
  });

  return (
    <Edit saveButtonProps={saveButtonProps} title="Edit Coupon">
